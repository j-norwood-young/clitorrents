...
- Session persistence in `session.json` — active transfers survive quit, crash, or restart
- Saves on every add, pause, resume, and remove (atomic writes; not only on quit)
- TUI and MCP restore transfers on startup with saved paths, magnets, categories, and pause state
- WebTorrent resumes partial downloads from existing files when the save path matches
- Results list badges for transfers already in progress (`[↓42%]`, `[‖]` paused, `[✓]` done)
- Results pagination with ← / → (one page per visible pane; ↑ / ↓ within the page)

### Changed

- Stay on the Results pane after adding a torrent (no auto-switch to Transfers)
- Removed route preview labels from the results list

### Fixed

- Block adding a torrent that is already downloading (info-hash match, including rapid double-Enter)
